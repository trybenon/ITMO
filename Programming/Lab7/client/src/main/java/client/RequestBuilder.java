...
public class RequestBuilder {
    ...
    public Request build(String line) {
        ...
        if (cmd.equals("add") || cmd.equals("add_if_max") || cmd.equals("update") || cmd.equals("remove_by_id") || cmd.equals("clear")|| cmd.equals("remove_head")) {
            if (ClientApp.user.getLogin() == null) {
                System.out.println("Ошибка: пользователь не авторизован. Пожалуйста, выполните аутентификацию.");
                return null;
            }
        }
        ...
        switch (cmd) {
            case "add" -> {
                ...
                return new Request(CommandType.ADD, new Object[]{p, ClientApp.user.getLogin()});
            }
            case "add_if_max" -> {
                Person p = clientManager.getPerson();
                return new Request(CommandType.ADD_IF_MAX, new Object[]{p, ClientApp.user.getLogin()});
            }
            case "login" -> {
                if (parts.length < 3) {
            ...
            case "update" -> {
                ...
                try {
                    long id = Long.parseLong(parts[1]);
                    return new Request(CommandType.CHECK_ID, new Object[]{id, ClientApp.user.getLogin()});
                } catch (NumberFormatException e) {
                    System.out.println("Ошибка: ID должен быть числом.");
                    return null;
                }
