...
public class RequestBuilder {
    /** Менеджер для интерактивного ввода полей Person */
    ...
    public RequestBuilder(ClientManager clientManager) {
    ...
    public Request build(String line) {
        if (ClientApp.isWaitingForInput) return null;
        ...
        switch (cmd) {
            case "add" -> {
                ...
                return new Request(CommandType.ADD, new Object[]{p});
            }
            case "add_if_max" -> {
                Person p = clientManager.getPerson();
                return new Request(CommandType.ADD_IF_MAX, new Object[]{p});
            }
            case "remove_by_id" -> {
                if (parts.length == 2) {
                    try {
            ...
            case "update" -> {
                ...
                try {
                    long id = Long.parseLong(parts[1]);
                    return new Request(CommandType.CHECK_ID, new Object[]{id});
                } catch (NumberFormatException e) {
                    System.out.println("Ошибка: ID должен быть числом.");
                    return null;
                }
            ...
            for (int i = 0; i < lines.size(); i++) {
                ...
                if (line.startsWith("execute_script")) {
                    ...
                    if (parts.length == 2) {
                        try {
                            long id = Long.parseLong(parts[1]);
                            sender.accept(new Request(CommandType.REMOVE_BY_ID, new Object[]{id}));
                        } catch (NumberFormatException e) {
                            System.out.println("Ошибка: ID должен быть числом.");
                        }
                    }
                ...
                else if (line.startsWith("update")) { // Изменили проверку на startsWith
                    String[] parts = line.split("\\s+", 2); // Разделяем строку на команду и ID
                    if (parts.length < 2) {
                        System.out.println("Нужно: update <id>");
                        break;
                    }
                    long id;
                    try {
                        id = Long.parseLong(parts[1]); // Извлекаем ID из той же строки
                    } catch (NumberFormatException e) {
                        System.out.println("Ошибка: ID должен быть числом.");
                        break;
                    }
    ...
    private Request buildSimple(String line) {
        switch (line) {
            ...
            case "info"                       -> { return new Request(CommandType.INFO, new Object[0]); }
            case "show"                       -> { return new Request(CommandType.SHOW, new Object[0]); }
            case "clear"                      -> { return new Request(CommandType.CLEAR, new Object[0]); }
            case "head"                       -> { return new Request(CommandType.HEAD, new Object[0]); }
            case "remove_head"                -> { return new Request(CommandType.REMOVE_HEAD, new Object[0]); }
            case "average_of_height"          -> { return new Request(CommandType.AVERAGE_OF_HEIGHT, new Object[0]); }
            case "print_ascending"            -> { return new Request(CommandType.PRINT_ASCENDING, new Object[0]); }
            case "print_field_ascending_height" -> { return new Request(CommandType.PRINT_FIELD_ASCENDING_HEIGHT, new Object[0]); }
            default -> {
