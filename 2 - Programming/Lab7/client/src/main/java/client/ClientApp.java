    ...
    private void read(SelectionKey key) throws IOException, ClassNotFoundException {
        ...
        if (dataBuffer == null) {
            ...
            try (ByteArrayInputStream bais = new ByteArrayInputStream(data);
                ...
                if (response.getMessage().startsWith("Приветствую")) {
                    String login = (String) response.getData();
                    if (user == null) {
                        user = new User();
                    }
                    user.setLogin(login);
                ...
                if (response.getMessage().startsWith("Найден")) {
                    ...
                    if (p != null) {
                        requestQueue.offer(new Request(CommandType.UPDATE, new Object[]{id, p, user.getLogin()}));
