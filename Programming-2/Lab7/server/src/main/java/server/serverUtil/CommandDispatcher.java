...
public class CommandDispatcher {
            ...
            try {
                switch (type) {
                    ...
                    case HEAD:
                        ...
                        break;
                    case REGISTRATION:
                        message = new RegistrationCommand(cm).execute(args);
                        break;
                    case AUTHENTICATE:
                        message = new AuthenticateCommand(cm).execute(args);
                        data = extractLogin(message);
                        break;
                    case REMOVE_HEAD:
    ...
    public static String extractLogin(String input) {
            if (input.startsWith("Приветствую ")) {
                return input.substring("Приветствую ".length());
            } else {
                return null;
            }
        }
